...
impl RenderState {
    ...
    pub async fn create(
        ...
        let adapter = {
            ...
            let mut found_adapter = instance
            ...
            // Portero VPN (parche local, ver vendor/egui-wgpu-0.29.1): en
            // algunas VMs sin GPU real ni Vulkan/DX12 utilizables, la
            // primera peticion (`force_fallback_adapter: false`) no
            // encuentra ningun adaptador -- ni siquiera WARP, el
            // renderizador por software de Windows, que en teoria deberia
            // estar siempre disponible. Se reintenta una vez pidiendolo
            // explicitamente como fallback antes de rendirse.
            if found_adapter.is_none() {
                log::info!(
                    "No se encontro ningun adaptador wgpu en el primer intento; reintentando con force_fallback_adapter=true"
                );
                found_adapter = instance
                    .request_adapter(&wgpu::RequestAdapterOptions {
                        power_preference: config.power_preference,
                        compatible_surface: Some(surface),
